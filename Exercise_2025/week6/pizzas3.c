...
int main () {
    ...
    while (1) {
        ...
        for (int i = 0; i < strlen(pizza_Set.ingredients); i++) {
            if (pizza_Set.ingredients[i] == ';') {
                count_of_ingredients++;
            }
        }
        if (count_of_ingredients > count) {
            count = count_of_ingredients;
            strcpy(max_pizza,pizza_Set.pizza_name);
        }
    }
